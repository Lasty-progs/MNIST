...
class Data:
    ...
    def __init__(self, mainDir):
        ...
        self.__private_currentDirs = [file for file in pathlib.Path(mainDir).iterdir() if file.is_dir()]
        

    def to_numpy(self, im):
        '''fastest way to convert Pil image to numpy massive (source of this solve: https://habr.com/ru/articles/545850/)'''
        im.load()
        # unpack data
...
class testData(Data):
    ...
    def make_dirs(self):
        ...
        try:
            ...
            for i in range(10):
                os.mkdir(self.__private_mainPath + "/results" + "/sorted_data/" + str(i))
        except:
...
class NeuralNetwork:
    ...
    # Activate func (for correct predictions)
    def __private_sigmoid(self, x):
        '''simple sigmoid func'''
    ...
    def predict(self, input):
        ...
        for instance, name in zip(input.data, range(len(input.data))):    
            out = []
            for i in range(10):
                out.append(self.__private_sigmoid(instance.flatten().dot(self.weights[i])))
            
            instance = instance.astype(numpy.uint8)
            out_img = Image.fromarray(instance)
            out_img = ImageOps.invert(out_img)
            out_img.save(self.__private_mainPath + "/results" + "/sorted_data/" + str(out.index(max(out))) + "/" + str(name) + ".jpg", "JPEG")
            

    def train(self, data, labels, epochs = 1, alpha = 1, visualize_weights = False):
        '''train model func(if you want to save visualisation weights, set last flag to true)'''
        ...
        for epoch in range(epochs):
            ...
            for instance in range(len(labels)):
                ...
                for number in range(10):
                    result = self.__private_trainPredict(data[instance], self.weights[number])
            ...
            if visualize_weights and epoch % 100 == 0: #it should be redone to increase performance
    ...
    def __private_show_weighs(self, epoch):
        ...
        for number in range(10):    
            ...
            for i in range(28):
                show_weights[i] = (self.__private_sigmoid(self.weights[number][28*i:28*(i+1)] * 2) * 256).astype(int)
            ...
            out_img.save(self.__private_mainPath + "/results" + "/visualized_weights/" + str(number) + "/" + str(epoch) + ".jpg", "JPEG")
    ...
    def __private_weight_folders(self):
        ...
        try:
            os.mkdir(self.__private_mainPath + "/results")
            os.mkdir(self.__private_mainPath + "/results" + "/visualized_weights/")
            for i in range(10):
                os.mkdir(self.__private_mainPath + "/results" + "/visualized_weights/" + str(i))
        except:
